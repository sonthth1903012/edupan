                ...
                <div class="col-12 col-lg-6">
                    <div class="single-top-popular-course d-flex align-items-center flex-wrap mb-30 wow fadeInUp" data-wow-delay="500ms">
                        <div class="popular-course-content">
                            <h5>{{$w->Post->title}}</h5>
                            <p>{{$w->Post->shortDesc}}</p>
                            <span>Location : {{$w->location}}</span>
                            <span>Time: {{$w->time}}</span>
                            <span>Attendees: {{$w->attendees}}/{{$w->capacity}}</span>
                            <span>Fee: {{$w->fee}}.000</span>
                            <a href="{{url("/workshop_detail",['id'=>$w->id])}}" class="btn academy-btn btn-sm">See More</a>
                        </div>
                        <div class="popular-course-thumb bg-img" style="background-image: url({{$w->Post->thumbnail}});"></div>
                    </div>
                </div>
                @endforeach
